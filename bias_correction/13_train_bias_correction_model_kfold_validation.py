...
# similar to ML_2.4 but performs K-fold cross validation over the TCCON stations

#TODO refractor to work with new path.py file
...
def precorrect(path, data):
    ...
    SA = data['SA'].to_numpy()
    # use B11 quality flag as place holder
    #qf = data['xco2_quality_flag'].to_numpy()
    qf1 = data['color_slice_noise_ratio_o2a'].to_numpy()
    qf2 = data['h_continuum_sco2'].to_numpy()
    # make bad quality flag [bad data = 1]
    qf = (qf1 >= 1.2) | (qf2 >= 5) # 1.5,10 values determined from cloud distance plots, filter similar to cld dist < 5km
    data.loc[:, 'xco2raw_SA_bias'] = calc_SA_bias_clean(XCO2, SA, qf)
...
def main(name, save_fig, mode, qf, max_n,  footprint, precorrect_IO):
    ...
    preload_IO = True
    data1 = load_data(2015, mode, qf=qf, preload_IO=preload_IO, footprint=footprint,  TCCON = TCCON)
    data2 = load_data(2021, mode, qf=qf, preload_IO=preload_IO, footprint=footprint,  TCCON = TCCON)
    data3 = load_data(2018, mode, qf=qf, preload_IO=preload_IO, footprint=footprint,  TCCON = TCCON)
    ...
    for t_name in TCCON_names:
        if data.loc[data['tccon_name'] == t_name, 'xco2tccon'].count() <= 100:
    ...
    for train_indx, test_indx in kf.split(TCCON_names):
        ...
        hold_out_year = 2020
        data = load_and_concat_years(2015, 2023, mode=mode, qf=qf, preload_IO=preload_IO, TCCON=TCCON, hold_out_year=hold_out_year)
        data_test = load_data(hold_out_year, mode, qf=qf, preload_IO=preload_IO, footprint=footprint, TCCON = TCCON)

        if filt is not None:
            data_test = data_test[(data_test['tccon_xluft'] - 0.999).abs() < filt]
            data_train = data_train[(data_train['tccon_xluft'] - 0.999).abs() < filt]
        ...
        if var_tp == 'xco2_TCCON_bias':
            ...
            #data_test[data_test['xco2tccon'] > 0]
        ...
        if precorrect_IO:
            # # ********** Correct data with first model **********************
            # hold out 2020 data
            data = load_data(2015, mode, qf=qf, preload_IO=preload_IO, footprint=footprint)
            data1 = load_data(2016, mode, qf=qf, preload_IO=preload_IO, footprint=footprint)
            data2 = load_data(2017, mode, qf=qf, preload_IO=preload_IO, footprint=footprint)
            data3 = load_data(2018, mode, qf=qf, preload_IO=preload_IO, footprint=footprint)
            data4 = load_data(2019, mode, qf=qf, preload_IO=preload_IO, footprint=footprint)
            data5 = load_data(2021, mode, qf=qf, preload_IO=preload_IO, footprint=footprint)
            data6 = load_data(2022, mode, qf=qf, preload_IO=preload_IO, footprint=footprint)
            data_train = pd.concat([data, data1, data2, data3, data4, data5, data6])

            data_test = load_data(2020, mode, qf=qf, preload_IO=preload_IO, footprint=footprint, TCCON=TCCON)
    ...
    if mode == 'all':
        for m in ['LndGL', 'LndND', 'SeaGL', 'all']:
            print(m)
            if m == 'LndND':
                print('removing ocean')
                d_m = data_test_all.loc[data_test_all['land_fraction'] == 100, :]
                ...
                print('removing land')
                d_m = data_test_all.loc[data_test_all['land_fraction'] == 0, :]
                d_m = d_m.loc[d_m['sensor_zenith_angle'] > 5, :]
            else:
                d_m = data_test_all
...
# make changes #############################
name = 'V6_6_k_filt0001_'  # name of generated model
...
n_splits = 5
# is this Steffen's computer
footprint = 0
mode = 'SeaGL' # Viewing Mode: 'LndGL', 'LndND', 'SeaGL', 'all'
precorrect_IO = False
filt = 0.001 # filter out data with tccon_xluft > filt
